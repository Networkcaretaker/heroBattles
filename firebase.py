...
def getRecordByName(dataset, recordID):
    data = db.collection(dataset).document(recordID).get()
    return (data)

# Get a sub-record from single record by ID
def getSubRecord(dataset, recordID, datasetB, record):
...
def getSubRecords(dataset, datasetB, record, order):
...
# Add a sub-record
def addSubRecord(dataset, data, datasetB, record):
